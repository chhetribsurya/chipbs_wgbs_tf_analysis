...
motifs_coord_df = final_motifs_model("ZBTB33_motifs_from_cpg_assoc_peaks.bed")
...
def load_cpg_pybedtool_object(file_name_with_full_path):
    ...
    cpg_df_select = pd.read_csv(file_name_with_full_path, sep="\t")
    #cpg_df_select = cpg_df[~(cpg_df["pval"] < 0.05)]
    cpg_df_select = cpg_df_select.iloc[:,0:8] # Removing the pval columns(10th col) from the file
    
    cpg_df_select["chip_meth"] = cpg_df_select["chipbs_rep1_meth"] 
    cpg_df_select["chip_unmeth"] = cpg_df_select["chipbs_rep1_unmeth"]
...
#tf_name_list = ["JunD", "Pol2", "TAF1", "YY1", "ZBTB33"]
tf_name_list = ["ZBTB33"]
