...
tf_name_list = ["ZBTB33"] # ZBTB has single replicate
...
for each_tf in tf_name_list:
    ...
    all_dfs = [wgbs_cpg_df, chipbs_rep1_df]
    ...
    merged_chipbs_wgbs_df_select = pd.read_csv(join(merged_chipbs_wgbs_file_dir, TF_name + "_chipbs_wgbs_merged_cpgs.bed"), sep="\t")
    ...
    df_ordered = df_file.iloc[:,[9,10,11,0,1,2,3,4,6,7]]
    df_ordered.columns = ["peak_chrom", "peak_start", "peak_end", "cpg_chrom", "cpg_start", "cpg_end", "wgbs_meth", "wgbs_unmeth", "chip1_meth", "chip1_unmeth"]

    df_ordered["chip_meth"] = df_ordered["chip1_meth"] 
    df_ordered["chip_unmeth"] = df_ordered["chip1_unmeth"]
    ...
    df_file = pd.read_csv(pybed_outfile, sep = "\t", header = None)
    cpg_ordered = df_file.iloc[:,[0,1,2,3,4,6,7]]
    cpg_ordered.columns = ["cpg_chrom", "cpg_start", "cpg_end", "wgbs_meth", "wgbs_unmeth", "chip1_meth", "chip1_unmeth"]

    cpg_ordered["chip_meth"] = cpg_ordered["chip1_meth"].astype(int) 
    cpg_ordered["chip_unmeth"] = cpg_ordered["chip1_unmeth"].astype(int) 
    ...
    ### Normalization for the wgbs ratio (round each fraction to 2 decimal places and replace 0 by 0.001):
    cpg_ordered["chip_meth_perc_norm"] = cpg_ordered["chip_meth_perc"].round(2).replace({0:0.1})
    cpg_ordered["wgbs_meth_perc_norm"] = cpg_ordered["wgbs_meth_perc"].round(2).replace({0:0.1})
